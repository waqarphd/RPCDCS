#uses "CMSfwFSMConfigDB/CMSfwFSMConfigDB_User.ctl"
...
void CMSfwFSMConfigDB_startRecipeMonitor()
{
  ...
  if (dynlen(exceptionInfo)){DebugTN("CMS Recipe Manager: DB connection error", exceptionInfo); exit (-1); }
...
//  DebugN("Starting monitor Thread for " + sPartition);
  ...
  while(true)
  {
    ...
    if (result == -1) {
      DebugTN("Verification not possible - aborting periodic check");
      return;
    }
...
//DebugN("Name of the recipe to verify: " + sRecipe);
  ...
  fwConfigurationDB_getRecipeFromCache(sSystemName + ":" +sRecipe, makeDynString(),"" ,recipeObject, exceptionInfo);
...
synchronized bool CMSfwFSMConfigDB_copyDBtoCache(string sRecipeName)
{
  dyn_string exceptionInfo;
  dyn_dyn_mixed recipeObject;
  dyn_string recipeList;
  
  if(dpExists("RecipeCache/" + sRecipeName))
    dpDelete("RecipeCache/" + sRecipeName);
  ...
  fwConfigurationDB_createRecipeCache(sRecipeName,exceptionInfo);
  fwConfigurationDB_storeRecipeInCache(recipeObject,sRecipeName,fwDevice_HARDWARE,exceptionInfo);
...
bool CMSfwFSMConfigDB_checkRecipeInCache(string sDeviceDp, string sRecipe)
{
  ...
  if(updateCache || !dpExists("RecipeCache/" + sRecipe) )
  {
//    DebugN("Loading " + sRecipe + " from database");
    if (! CMSfwFSMConfigDB_copyDBtoCache(sRecipe)) {
//     	DebugTN("Recipe not found in the DB");
     	return false;
    }
    else
    {
...
//  DebugN("CMSfwFSMConfigDB_DULoad",CMSfwFSMConfigDB_abort[device]);    
  ...
  applyResult = CMSfwFSMConfigDB_applyRecipeFromCache(sRecipe,recipeFromCache);
...
bool CMSfwFSMConfigDB_applyRecipeFromCache(string sRecipe,dyn_dyn_mixed& recipeObject,dyn_string aDeviceList = makeDynString())
{
  dyn_string exceptionInfo;
...
//DebugN("apply recipe from cache", sRecipe, aDeviceList);
  fwConfigurationDB_getRecipeFromCache(sRecipe, aDeviceList,"",recipeObject,exceptionInfo);
  if (dynlen(exceptionInfo) > 0)
  {
    DebugN(exceptionInfo);
    return false;
