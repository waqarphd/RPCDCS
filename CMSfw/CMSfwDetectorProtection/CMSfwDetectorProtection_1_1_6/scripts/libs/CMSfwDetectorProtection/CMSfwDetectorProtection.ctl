...
const string CMSfwDetectorProtection_managerDp = "CMSfwDetectorProtection/CtrlManager";
const string CMSfwDetectorProtection_managerDpReplies = "CMSfwDetectorProtection/CtrlManagerCheckReply";
...
void CMSfwDetectorProtection_connect(dyn_mixed& object, dyn_string& exceptionInfo) {
  ...
  for (int i=1; i<= dynlen(object[CMSfwDetectorProtection_CONDITIONNAME]); i++) {
    ...
    remotesys = CMSfwDetectorProtection_getInputSystem(object,i);
    CMSfwDetectorProtection_connectSystem(remotesys);
  }
...
void CMSfwDetectorProtection_connectSystem(string sysName) {  
  ...
  if (index == 0) {
    ...
    CMSfwDetectorProtection_systemConnected[index] = true; // will be updated immediately after
...
synchronized void CMSfwDetectorProtection_systemConnectionChanged(string dpe, bool connected) {
  dpe = dpSubStr(dpe, DPSUB_SYS_DP_EL);
  string sysName = dpSubStr(dpe, DPSUB_DP);
  ...
  if (! connected) {
    ...
    CMSfwDetectorProtection_disconnectionTime[index] = t;
  }
...
synchronized void CMSfwDetectorProtection_recompute(dyn_mixed& object, dyn_string maskedDpes) {
  ...
  for (int i=1; i<= dynlen(object[CMSfwDetectorProtection_CONDITIONNAME]); i++) {
    ...
    toRecompute = dynContains(CMSfwDetectorProtection_conditionsToRecompute, i);
  ...
  /*
    ...
    outputdpes =  CMSfwDetectorProtection_getDpesLockedByCondition(i);
    
    if (! object[CMSfwDetectorProtection_FIRED][i]) {
    idxDpeIndexes = idxDpeIndexes + dynlen(outputdpes);
    continue;
    }
    //    CMSfwDetectorProtection_debug4("Condition " + i + ":" + dynlen(outputdpes) + " output dpes"); //DebugN(outputdpes);
    for (int j=1; j<= dynlen(outputdpes); j++) {
    index = CMSfwDetectorProtection_outindexes[idxDpeIndexes];
    idxDpeIndexes++;
    lock[index] = true;      
    if (settingCondition[index] != 0) {
    if ( CMSfwDetectorProtection_getOutputValue(object,settingCondition[index],cache) != CMSfwDetectorProtection_getOutputValue(object,i, cache)) {
    CMSfwDetectorProtection_debug1("**** CRITICAL ERROR : Two fired conditions ask to set dpe " + outputdpes[j] + " to different values (" 
    + CMSfwDetectorProtection_getOutputValue(object,settingCondition[index], cache) + " given from "
    + " condition " +object[CMSfwDetectorProtection_CONDITIONNAME][settingCondition[index]] + " (" + index + ") != " + CMSfwDetectorProtection_getOutputValue(object,i, cache)+ ")" + 
    " while processing condition " + object[CMSfwDetectorProtection_CONDITIONNAME][i] + " ( " + i + " ) ");                    
    }
    }
    settingCondition[index] = i;                 
    }
    }
  */
      ...
      switch  (CMSfwDetectorProtection_getPureMode(object[CMSfwDetectorProtection_OUTPUTMODE][index])) {
          ...
          if (dynlen(settings) == dynlen(verif)) {
              ...
              for (int i=1; i<= dynlen(verif); i++) {
                  if (dpSubStr(verif[i], DPSUB_SYS_DP) == dpe) return makeDynString(verif[i]);
              } 
          } else {
...
void CMSfwDetectorProtection_start() {  
  ...
  dpConnect("CMSfwDetectorProtection_updateIgnoreInvalid", CMSfwDetectorProtection_managerDp + ".ignoreInvalidBit");
  dpConnect("CMSfwDetectorProtection_updateMaskedDpes",  
              CMSfwDetectorProtection_managerDp + ".mask.maskedDpes");
  dpSet(CMSfwDetectorProtection_managerDp + ".consistency.invalid", false);
  
  dpSetDescription(CMSfwDetectorProtection_managerDp + ".consistency.invalid", "Det. Protection mapping invalid on " + getSystemName());
  string alertClass = "_fwFatalNack_90.";
  fwAlertConfig_setDigital(CMSfwDetectorProtection_managerDp + ".consistency.invalid", makeDynString("", "Detector protection mapping invalid on system "  + getSystemName() + " please restart it when possible!"), makeDynString("", alertClass),
                           "",makeDynString(),"", exc);
  
  
  fwAlertConfig_activate(CMSfwDetectorProtection_managerDp + ".consistency.invalid", exc);
  ...
  alertClass = "_fwWarningNack_50.";
    fwAlertConfig_setDigital(CMSfwDetectorProtection_managerDp + ".ignoreInvalidBit", makeDynString("", "Invalid bit ignored by Detector Protection on "  + getSystemName() ), makeDynString("", alertClass),
                           "",makeDynString(),"", exc);
  
  
  fwAlertConfig_activate(CMSfwDetectorProtection_managerDp + ".ignoreInvalidBit", exc);
...
dyn_string CMSfwDetectorProtection_getMaskedDpesForSystem(string systemName) {
    string dp = systemName + CMSfwDetectorProtection_managerDp + ".mask.maskedDpes";
...
bool  CMSfwDetectorProtection_verifyRemote(string replydp, string systemName, dyn_string& problems, dyn_string& exceptionInfo) {
  ...
  string dpIgnoreInvalid =  systemName + CMSfwDetectorProtection_managerDp + ".ignoreInvalidBit";          
...
bool CMSfwDetectorProtection_setLocked(dyn_string dpes, int locked) {
  CMSfwDetectorProtection_dpMassSet(dpes,":_lock._original._locked",locked);
}
...
bool CMSfwDetectorProtection_isManagerRunning(string systemName, time t = getCurrentTime()) {
  string dpe  = systemName + CMSfwDetectorProtection_managerDp + ".aliveTime";
...
dyn_string CMSfwDetectorProtection_getRequiredPrivileges(string systemName) {
  ...
  string priv_dpe = systemName + CMSfwDetectorProtection_managerDp + ".accessControl.requiredPrivileges";
...
bool CMSfwDetectorProtection_setRequiredPrivileges(string systemName, dyn_string requiredPrivileges) {
  string priv_dpe = systemName + CMSfwDetectorProtection_managerDp;
