...
synchronized checkAlerts (string ident, dyn_dyn_anytype val) {       
  ...
  for (int i=1; i <= dynlen(val);i++){
    ...
    if(!fwInstallationRedu_isPassive())  //Change to make it redu compatible  
    _CMSfwAlertSystem_processAlert(dp, iAlert);   
  }  
}

void _CMSfwAlertSystem_processAlert (string dp, int iAlert){  
//  DebugN("[CMSfwAlertSystem][_CMSfwAlertSystem_processAlert] Notification for " + dp + " " + iAlert);
  ...
  for (int i=1; i <= dynlen(dsUsers); i++){
    ...
    if (dynlen(notificationSent) < dynlen(notifications)) {
      for (int j= dynlen(notificationSent) +1; j <= dynlen(notifications); j++) {
        notificationSent[j] = false;
      }
      dpSet(dsUsers[i] + ".NotificationSent", notificationSent);
    }
...
CMSfwAlertSystemGuardian_sendEmail (string dp1,  string userName,
  ...
  for(int alertNum=1 ; alertNum <= dynlen (sumAlerts); alertNum++){
    ...
    emailMessage+= CMSfwAlertSystemGuardian_formatText(  dp1,  userName   ,
                                                         dp2,  text       ,
                                                         dp3,  sumAlerts  );
  }
  //if there is an alert  
...
// text to send:
////////////////////////////////////////////////////////////////////////////////
string CMSfwAlertSystemGuardian_formatText(  string dp1,  string userName,
                    string dp2,  string text,
                    string dp3 , dyn_atime sumAlerts) {
  ...
  for(int alertNum=1 ; alertNum <= dynlen (sumAlerts); alertNum++){
    ...
    anytype val;
    string unit;
    string dpe = dpSubStr(getAIdentifier(sumAlerts[alertNum]),DPSUB_SYS_DP_EL);
    ...
    if (dpElementType(dpe) != DPEL_BOOL) {
        ...
        int range;
        dpGet(sAlertDp + ":_alert_hdl.._num_ranges", range);
      
        if(range == 2) {
            dpGet(sAlertDp + ":_alert_hdl.1._l_limit", lowerLimit); 
            dpGet(sAlertDp + ":_alert_hdl.1._u_limit", upperLimit); 
            
            if(val >= upperLimit) {
              // In this case, the first range is a GOOD range.
              emailMessage+= dpGetDescription(sAlertDp) +"= "+ val+" >= "+upperLimit+unit+" (upper bound exceeded); ";
            } else if (val <= upperLimit) {
              // In this case, the first range is a BAD range.
              emailMessage+= dpGetDescription(sAlertDp) +"= "+ val+" <= "+upperLimit+unit+" (lower bound exceeded); ";
            } // otherwise it's good range.
        } if(range == 3) {
            // In this case, the second range is a GOOD range.
            dpGet(sAlertDp + ":_alert_hdl.2._l_limit", lowerLimit); 
            dpGet(sAlertDp + ":_alert_hdl.2._u_limit", upperLimit); 
            
            if(val >= upperLimit) {
              emailMessage+= dpGetDescription(sAlertDp) +"= "+ val+" >= "+upperLimit+unit+" (upper bound exceeded); ";
            } else if (val <= lowerLimit) {
              emailMessage+= dpGetDescription(sAlertDp) +"= "+ val+" <= "+lowerLimit+unit+" (lower bound exceeded); ";
            } // otherwise it's good range.
        } // otherwise do not handle complex ranges.
      

     } else{
      // Alarm raised over a boolean field.      
      emailMessage+= dpGetDescription(sAlertDp) +" "+ value + " (bool alert activated); ";
    }
  }
