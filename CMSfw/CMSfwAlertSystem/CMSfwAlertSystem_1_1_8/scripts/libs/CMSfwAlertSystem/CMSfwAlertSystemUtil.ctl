...
void CMSfwAlertSystemUtil_setToInvalid(string configurationName, string dpname_pattern = "CMSAlertSystem") {
  dyn_string exc;
  ...
  dyn_string exc;
  CMSfwAlertSystemUtil_setValidTo(aCitemID,iId, exc);
  if (dynlen(exc)>0) {
    if (myManType() == UI_MAN) {
...
void CMSfwAlertSystemUtil_setValidTo(dyn_string aCitemID,int iId, dyn_string& exc, time t0 = getCurrentTime())
{
  int rc;
...
bool CMSfwAlertSystemUtil_saveToConfigurationDb(string configurationName = "") {
 ...
 if (dynlen(exceptionInfo)>0) {
  ...
  CMSfwAlertSystemUtil_setToInvalid(configurationName);
...
bool CMSfwAlertSystemUtil_loadFromConfigurationDb(string configurationName = "") {
  ...
  if (myManType() ==  UI_MAN) {
    ...
    CMSfwAlertSystemUtil_request("loadFromConfigurationDb", makeDynString(confNameOrig),1200); return true;
  }   
