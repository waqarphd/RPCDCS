...
calcHumfromdewP()
{
  ...
  {  
    ...
    if(dynlen(dpel)==2)
    {
      ...
      { 
        dpGet(dpel[2]+".value",dewP,dpel[1]+".value",temp);  
      }
...
{
  int error=0;
  string dpelset = dpNames(sysName+"*Barrel*", "RPCGasStatus");
  dyn_string barrel = makeDynString("Rack69", "Rack70", "Rack71", "Rack72", "Rack73", 
...
endcaprefresh()
{
  int error=0, warning=0;
  string valueget;
  string dpelset = dpNames(sysName+"*Endcap*", "RPCGasStatus");
  dyn_string endcap = makeDynString("Rack65", "Rack62", "Rack63", "Rack66", "Rack67", 
...
infrastructurerefresh()
{
  int error=0, warning=0;
  string valueget;
  string dpelset = dpNames(sysName+"*Infrastructure*", "RPCGasStatus");
...
statusrefresh()
{
  ...
  for(int i = 1; i<= dynlen(chamberEndcap); i++)
  {
    ...
    if(errorNr > 15)
    { 
      dpSet(sysName+chamberEndcap[i]+".status", 2);
    }
    else
    {
      if(warningNr > 15) 
      {
        dpSet(sysName+chamberEndcap[i]+".status", 1);
      }
      else
      {
        if(unacknowledgeNr > 15) 
        {
          dpSet(sysName+chamberEndcap[i]+".status", 2);
        }
        else 
        {
          dpSet(sysName+chamberEndcap[i]+".status", 0);
        }
      }
    }
  ...
  for(int i = 1; i<= dynlen(chamberBarrel); i++)
  {
    ...
    if(errorNr > 15)
    { 
      dpSet(sysName+chamberBarrel[i]+".status", 2);
    }
    else
    {
      if(warningNr >15)
      { 
        dpSet(sysName+chamberBarrel[i]+".status", 1);
      }
      else
      {
        if(unacknowledgeNr >15)
        ...
        { 
          dpSet(sysName+chamberBarrel[i]+".status", 0);
        }
      }
    }
...
//      
    ...
    if(errorNr != 0)
    { 
      dpSet(sysName+RackNr[i]+".status", 2);
    }
    else
    {
      if(warningNr !=0)
      { 
        dpSet(sysName+RackNr[i]+".status", 1);
      }
      else
      {
        if(unacknowledgeNr !=0) 
        {
          dpSet(sysName+RackNr[i]+".status", 2);
        }
        else 
        {
          dpSet(sysName+RackNr[i]+".status", 0);
        }
      }
    }
