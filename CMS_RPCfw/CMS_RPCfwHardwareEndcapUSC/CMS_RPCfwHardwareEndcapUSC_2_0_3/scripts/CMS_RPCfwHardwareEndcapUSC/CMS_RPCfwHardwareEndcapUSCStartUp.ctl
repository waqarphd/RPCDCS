...
systemCheck()
{
  ...
  if(!dpExists(getSystemName()+"RPCCheckUSCE"))
  {
    ...
    string dp = getSystemName()+"RPCCheckUSCE.fvalue";
    fwArchive_set(dp,"RDB-99) EVENT",DPATTR_ARCH_PROC_SIMPLESM,DPATTR_COMPARE_OLD_NEW,0,0,exInfo);
    fwAlertConfig_set(dp, DPCONFIG_ALERT_NONBINARYSIGNAL, alertTexts, limits, alertClasses, 
                      summary, alertPanel, alertPanelParameters, alertHelp,exInfo);
