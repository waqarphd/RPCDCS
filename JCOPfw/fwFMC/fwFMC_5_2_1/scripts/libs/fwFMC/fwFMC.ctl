...
const string FW_FMC_LIB_VERSION = "5.2.1";
...
//DebugN(">>>>>>>>>>>>>>>>>active = " + active + "  archivingExists = " + fwFMC_archivingExists(node, exceptionInfo));      
  ...
  { 
    ...
    dpes=makeDynString();
    canHaveArchive=makeDynBool();
    dyn_string monitoredProcesses = fwFMCProcess_getMonitoredProcesses(node,exceptionInfo);
    if(dynlen(monitoredProcesses)>0){
      dynAppend(deviceDpNames, monitoredProcesses);
