...
{
  if(dynlen(pidArr) != dynlen(memoryArr) ||
     dynlen(pidArr) != dynlen(cpuArr) ||
     dynlen(pidArr) != dynlen(nameArr) ||
     dynlen(pidArr) != dynlen(cmdLineArr) ||
     dynlen(pidArr) == 0)
    return;
  //get the node name
  ...
  for (int i=1; i <= dynlen(monitoredProcesses);i++)
  {
    ...
    dpSet(monitoredProcesses[i] + ".readings.pid", pid,
          ...
          monitoredProcesses[i] + ".readings.state", ((int)found + 1));    
  }
}
...
void fwFMCProcess_startMonitoringProcess(string node, int pid, string name, string options)
{
  ...
  if(!processMonitored) // new process to monitor
  {
    ...
    if (dpCreate(dpName, FW_FMC_MONITORED_PROCESS_DPT) != -1)
    {
      ...
      //set alarm
      fwFMCProcess_setupAlarm(dpName);
...
void fwFMCProcess_setupAlarm(string monitoredProcessDp)
{
   dyn_string ex;
  //Find out the archive class to be used:
  dyn_string dpes, configTypes, alertPanel, help;
  dyn_dyn_string texts, classes, alertPanelParameters, summaryDpeList;
  dyn_dyn_float limits;
  _fwFMC_getAlarmsSettings(monitoredProcessDp, dpes, 
                           configTypes, texts, 
                           limits, classes, 
                           summaryDpeList, alertPanel, 
                           alertPanelParameters, help, ex) ;
   for(int i=1; i<=dynlen(dpes); i++)
   {
     fwAlertConfig_setAnalog(dpes[i], texts[i], limits[i],classes[i], alertPanel[i], 
                             alertPanelParameters[i],  help[i], ex);
     if(dynlen(ex) == 0)
