...
const string FW_FMC_TOOL_VERSION_DPT = "5.0.4";
const string FW_FMC_LIB_VERSION = "5.0.4";

const string FW_FMC_INSTALLATION_TOOL_VERSION = "4.2.1";
const string FW_FMC_SYSTEM_CONFDB_VERSION = "4.1.1";
...
void fwFMC_setDefaultAlarms(string node, dyn_string &exceptionInfo, bool active = TRUE)
{
  ...
  { 
    if(dynlen(fwFMCProcess_getMonitoredProcesses(node,exceptionInfo))>0)
    {
      ...
      for(int i = 1; i <= dynlen(procs); i++)            
        dynAppend(deviceDpNames, fwFMCProcess_getDp(node) + "/MonitoredProcesses/" + procs[i]);
  ...
  { 
    ...
    if(exceptionInfo != "") DebugTN(exceptionInfo);
    dynAppend(deviceDpNames, deviceFSDpNames);   
  }
...
{
  dyn_string nodes;
  fwNode_getNodes(getSystemName(), fwNode_TYPE_ALL,  nodes, exceptionInfo);
  if(dynContains(nodes, getSystemName() + FW_FMC_LOGICAL_VENDOR) <= 0)
  {
...
void fwFMC_dimManagersCB(string connectionsDpe, dyn_int manNums)
{
  dyn_string dpes;
  dyn_int states;
  dyn_string nodes;
  ...
  if(dynContains(manNums, FW_FMC_PROCESS_DIM) < 0) //dim api is not running
  {
    ...
    {
      if(fwFMCProcess_exists(nodes[i]))
        dynAppend(dpes, getSystemName() + "FMC/"+nodes[i]+".agentCommunicationStatus.process");
    }
    int n = dynlen(dpes);
    for(int i = 1; i <= n; i++)
      dynAppend(states, -2);
  ...
  if(dynContains(manNums, FW_FMC_IPMI_DIM) <= 0) //dim api is not running
  {
    for(int i = 1; i <= dynlen(nodes); i++)
    {
      if(fwFMCIpmi_exists(nodes[i]))
        dynAppend(dpes, getSystemName() + "FMC/"+nodes[i]+".agentCommunicationStatus.ipmi");
    }
    int n = dynlen(dpes);
    for(int i = 1; i <= n; i++)
      dynAppend(states, -2);
  ...
  if(dynContains(manNums, FW_FMC_TM_LOGGER_DIM) <= 0) //dim api is not running
  {
    for(int i = 1; i <= dynlen(nodes); i++)
    {
      if(fwFMCTaskManager_exists(nodes[i]))
        dynAppend(dpes, getSystemName() + "FMC/" + nodes[i] + ".agentCommunicationStatus.taskManager");
      
      if(fwFMCLogger_exists(nodes[i]))
        dynAppend(dpes, getSystemName() + "FMC/" + nodes[i] + ".agentCommunicationStatus.logger");
    }
    int n = dynlen(dpes);
    for(int i = 1; i <= n; i++)
      dynAppend(states, -2);
  ...
  if(dynContains(manNums, FW_FMC_CONTINUOUS_MONITORING_DIM) <= 0) //dim api is not running
  {
    for(int i = 1; i <= dynlen(nodes); i++)
    {
      if(fwFMCMonitoring_exists(nodes[i]))
      {        
        dynAppend(dpes, dpNames(getSystemName() + "FMC/" + nodes[i] + ".agentCommunicationStatus.monitoring.mem", "FwFMCNode"));
        dynAppend(dpes, dpNames(getSystemName() + "FMC/" + nodes[i] + ".agentCommunicationStatus.monitoring.cpuinfo", "FwFMCNode"));
        dynAppend(dpes, dpNames(getSystemName() + "FMC/" + nodes[i] + ".agentCommunicationStatus.monitoring.cpustat", "FwFMCNode"));
      }
    }
    int n = dynlen(dpes);
    for(int i = 1; i <= n; i++)
  ...
  if(dynContains(manNums, FW_FMC_TEMPORARY_MONITORING_DIM) <= 0) //dim api is not running
  {
    for(int i = 1; i <= dynlen(nodes); i++)
    {
      if(fwFMCMonitoring_exists(nodes[i]))
      {        
        dynAppend(dpes, dpNames(getSystemName() + "FMC/" + nodes[i] + ".agentCommunicationStatus.monitoring.os", "FwFMCNode"));
        dynAppend(dpes, dpNames(getSystemName() + "FMC/" + nodes[i] + ".agentCommunicationStatus.monitoring.nif", "FwFMCNode"));
        dynAppend(dpes, dpNames(getSystemName() + "FMC/" + nodes[i] + ".agentCommunicationStatus.monitoring.fs", "FwFMCNode"));
      }
    }
    int n = dynlen(dpes);
    for(int i = 1; i <= n; i++)
...
/*
fwFMC_updateTimeStamp(string fmcNodeName, time timeFMC = 0){
  
  if(timeFMC == 0){
    if(dpExists(fmcNodeName + ".config.lastUpdate")){
        dpSet(fmcNodeName + ".config.lastUpdate", getCurrentTime());
    }
    else{
   	dpSet(fmcNodeName + ".lastUpdate", getCurrentTime());
    }
  }
  else{
    if(dpExists(fmcNodeName + ".config.lastUpdate")){
	dpSet(fmcNodeName + ".config.lastUpdate", timeFMC);
    }
    else{
    }
  }
...
*/
/*
bool fwFMC_isManagerAlive(int manNum = 0, string group = ""){

  bool managerStatus = FALSE;
  time timeZero, timeAliveCtrl;

  dpSet("fwFMC.timeAliveCtrl", 0);
  dpGet("fwFMC.timeAliveCtrl", timeZero);

  dpSet("fwFMC.ctrlID", manNum,
        "fwFMC.groupName", group,
        "fwFMC.actionCtrl", "isAlive");

  delay(0,200);

  dpGet("fwFMC.timeAliveCtrl", timeAliveCtrl);
		
  if(timeAliveCtrl > timeZero){
    managerStatus = TRUE;
  }

  if( !managerStatus && (group != "") && (manNum == 0)){
    if(dpExists(group)){
      dpSet(group + ".managerCtrlID", 0,
            group + ".managerCtrlBool", FALSE);
    }
  }
...
string fwFMC_getNodeName(string dp)
{
  ...
  if(type == FW_FMC_NODE_DPT || type == FW_SYSTEM_OVERVIEW_NODE_DPT)
    ...
    i = dynlen(ds) - 1;
  else if(type == FW_FMC_MONITORING_CPU_BASE || type == FW_FMC_MONITORING_NETWORK_BASE || type == FW_FMC_MONITORING_FS_BASE || type == FW_FMC_MONITORING_MEMORY || type == FW_FMC_MONITORED_PROCESS_DPT || type == FW_FMC_MONITORED_SERVICE_DPT)
    i = dynlen(ds) - 2;
  else if (type == FW_FMC_MONITORING_FS)
    i = dynlen(ds) - 3;
  return ds[i];  
}
