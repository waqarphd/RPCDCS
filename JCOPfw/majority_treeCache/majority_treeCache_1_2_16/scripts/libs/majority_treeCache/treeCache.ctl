...
void treeCache_initIncludedCache(string root, string dont_go_below_type="", bool isIdx = false) {
  ...
  dyn_string dps; string includedDp;
  int idx = 1;
  for (int i=1; i<= dynlen(visited); i++) {
    if (strlen(treeCache_getType(visited[i], true))>0) { // not a "system node"
      includedDp = treeCache_getIncludedDp(treeCache_getNode(visited[i]));
      if (dpExists(includedDp)) {
        dps[idx] = includedDp;
        idx++;
      } else {
        DebugTN("Skipping non existing dp " + includedDp);          
      }
    } else {
       DebugN("Skipping node with empty type " + treeCache_getNode(visited[i]) + " (" + visited[i] + ")") ;         
    }
  }
...
/*
  ...
  @node topNode
  @excludedFsmTypes list of fsm types where to stop the caching or list of nodes in the form node=<node>
         ...
         if (treeCache_getUnit(n, true) == "CU") {
             ...
             if (! dpExists(includeddp) ) {
                all_exist = false;
                   ...
                   bool configExists; dyn_string functionParams; 
                			dyn_string functionGlobals; string functionDefinition;  dyn_string exceptionInfo;
                
                   fwDpFunction_getDpeConnection(includeddp,  configExists, functionParams, 
                								functionGlobals, functionDefinition, exceptionInfo);                    
             
                   if ((! configExists) || (dynlen(functionParams) == 0) || (! dpExists(functionParams[1]))) {
                     treeCache_debug1("treeCache: function in dp " + includeddp + " is not set properly. Removing dp");
                     dpDelete(includeddp);
                     all_exist = false;                     
                   }   
...
int treeCache_initIncluded(string topNode, bool dummyOnlyCU = false, bool dummy2=false, string systemName = "") {
  ...
  for (int n=start; n<= stop; n++) {
    ...
    if (dynlen(exceptionInfo) >0 ) {
      ...
      return _treeCache_error("Unable to set the dp function in the included dp for node " + node + " linking to dpe " + dpe + " that is " + (dpExists(dpe)?"existing":"not existing"));
...
int _treeCache_visit(string node,int&Nnodes,int&Nlevels,int&Nsystems,int&NlevelsInSystem,
  ...
  if (dynContains(excludedFsmTypes, "node=" + node)) { // ignore the node if in the list of types it is explicitly listed
    return 1;
  }
