...
global mapping        majority_mapAnyDpe;
...
int majority_createVisit(int nodeIdx,dyn_string percentages,int pollLoop,int pollTime,int pollMax, string systemName = "", bool check = false) {
  ...
  if (writeToDp) {
    ...
    if (! check) {
      ...
      majority_setDpe(dp + ".admin.topNode",majority_topNode);
    }
  }
...
void majority_startVisit(int nodeIdx) {
  ...
  majority_getDpe(dp + ".admin.topNode", relatedTopNode);
  ...
  if ( unit != "DU" ) {
    ...
    bool connect = false;
    int devSelected = majority_isConnectingToFsmType(node, fsmType);
    connect = (devSelected != -1);
...
int majority_isConnectingToFsmType(string node, string fsmType) {   
...
void majority_updateWithNoDeltas(string node, int nodeIdx, bool calcTotal = false) {
  ...
  do {
	...
	for (int c= 1; c<= dynlen(children); c++) {	          
	  ...
	  majority_getDpe(majoritydp + ".admin.topNode", relatedTopNode);	  
  ...
  /////** If the node that we are updating is an FSM type that is counted by the majority, tot_counts will report 0 for that device
  ///// in this case we have to overwrite tot_counts with counts for the positions relative to this device
  
  int devSelected = majority_isConnectingToFsmType(parent, treeCache_getType(parentIdx, true));
  if (devSelected > -1) {
    if (isAll) {
      tot_counts[devSelected] = counts[devSelected];
    } else {
       for ( int s = dynlen(majority_devicesXpercIdx[devSelected]) ; s ; s -- ) {
	        tot_counts[majority_devicesXpercIdx[devSelected][s]] =   counts[majority_devicesXpercIdx[devSelected][s]];
	      }
    }
  }
...
void  majority_getDpe(string dpe, anytype& value) {
  if (mappingHasKey(majority_mapAnyDpe, dpe)) {
      value =  majority_mapAnyDpe[dpe];
      return;
  }
  dpGet(dpe, value);
  majority_mapAnyDpe[dpe] = value;
}

void majority_setDpe(string dpe, anytype value) {
    dpSetWait(dpe, value);
    majority_mapAnyDpe[dpe] = value;
}
