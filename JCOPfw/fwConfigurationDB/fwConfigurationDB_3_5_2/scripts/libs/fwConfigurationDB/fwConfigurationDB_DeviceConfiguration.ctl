...
@date   January 2012
*/
global string _fwConfigurationDB_fileVersion_fwConfigurationDB_DeviceConfiguration_ctl="3.5.2";
...
DebugTN("final filtering starts...");	
...
DebugTN("final filtering done",dynlen(dpeList));	    
	}
    }
...
void _fwConfigurationDB_savePropAddressToDB(dyn_string dpes,dyn_int ipropIds, dyn_string &exceptionInfo)
{
    ...
    for (int i=1;i<=dynlen(addrCfg);i++) {        
        if (configExists[i]) {
    	    ...
    	    for (int j=FW_PARAMETER_FIELD_LOWLEVEL;j<=dynlen(addrCfg[i]);j++) {
        	dynAppend(options,(string)addrCfg[i][j]);
    	    }
