...
global string _fwConfigurationDB_fileVersion_fwConfigurationDB_Hierarchies_ctl="3.3.65";
...
if (options & fwConfigurationDB_deviceConfig_NO_DEVICE_CREATE) {
    DebugTN("CONFIGURE WITHOUT RE-CREATING DEVICES: Filtering the list of properties");
    // simply remove the data for the missing devices (already listed 
    // in missingDeviceListObject) from the deviceListObject;

//    DebugTN("Missing devices",missingDeviceListObject[fwConfigurationDB_DLO_DPNAME]);
//    DebugTN("Full list",deviceListObject[fwConfigurationDB_DLO_DPNAME]);
//    DebugTN("Before modifying deviceListObject is",deviceListObject);
    dyn_string missingDevNames=missingDeviceListObject[fwConfigurationDB_DLO_DPNAME];
    dyn_string allDevNames=deviceListObject[fwConfigurationDB_DLO_DPNAME];
    for (int i=1;i<=dynlen(missingDevNames);i++) {
	int idx=dynContains(allDevNames,missingDevNames[i]);
	if (idx) {
	    // we need to remove it...
	    dynRemove(allDevNames,idx);
	    dynRemove(missingDevNames,i);
	    
	    // (!) remove the selected ipropids from the temp table(!)
	    string sqlForTempTable="DELETE FROM TMP_ITEM_PROPS WHERE "+_fwConfigurationDB_listToSQLString("iprop_id", deviceListObject[fwConfigurationDB_DLO_PROPIDS][idx]);
//DebugTN("We are going to SQL",sqlForTempTable);
	    fwConfigurationDB_executeSqlSimple(sqlForTempTable, g_fwConfigurationDB_DBConnection, exceptionInfo);                                                                              
	    if (dynlen(exceptionInfo)) {rdbRollbackTransaction(g_fwConfigurationDB_DBConnection);return;}
	    
	    // now remove also from the deviceListObject, yet from all the columns!
	    for (int k=1;k<=fwConfigurationDB_DLO_MAX_IDX;k++) {
		dynRemove(deviceListObject[k],idx);
	    }
	// mitigate the iterator, because we removed the item:
	i--;
	}
    }
//    DebugTN("After modifying deviceListObject is",deviceListObject);
    DebugTN("CONFIGURE WITHOUT RE-CREATING DEVICES: Filtering done");
...
//DebugTN("SupportedConfigs",dpeSupportedConfigs);
...
//DebugTN("Got values",propIds,valDpes);
        if (dynlen(exceptionInfo)) return;
