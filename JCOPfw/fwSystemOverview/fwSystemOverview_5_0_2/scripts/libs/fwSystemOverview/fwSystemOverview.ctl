...
const string FW_SYSTEM_OVERVIEW_TOOL_VERSION = "5.0.2";
                                ...
                                string &managerDp)
{
  ...
  for(int i = 1; i <= n; i++)
  {
    ...
    if(managerType == pvssManagersInfo[2][i] &&
       ((managerType == "WCCILpmon"   ||
        managerType == "WCCILdata"   ||
        managerType == "WCCILevent"  ||
        managerType == "WCCILdist"   ||
        managerType == "WCCOArdb"     ) ||
       ...
       (managerOptions != "" && patternMatch("*-num " + managerNum + " *", pvssManagersInfo[3][i]) && managerIndex == pvssManagersInfo[4][i]) || 
...
//calculate project state and create the managers DPs if they don't exist
  ...
  for(int i = 1; i <= dynlen(ddsResult); i++)
  {
    ...
    if (!fwSysOverview_managerStateAlarmMasked(projDp, ddsResult[i], ddsResultStati[i][5], (i-1)))
...
bool fwSysOverview_managerStateAlarmMasked(string projDp, dyn_string managerConfig, int manNum, int manIndex)
{
  bool masked = false;
