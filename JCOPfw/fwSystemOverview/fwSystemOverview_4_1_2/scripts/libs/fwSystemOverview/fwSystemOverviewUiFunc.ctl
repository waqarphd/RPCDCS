...
int fwSystemOverviewUiFunc_connectDp2Ui(string workFuncSys, string workFuncProj, bool firstUpdate = TRUE)
{
  ...
  for(i = 1; i <= dynlen(sysDpList); i++){
    ...
    dpConnect(workFuncSys, firstUpdate, sysDpList[i] + fwSystemOverviewFunc_CURR_STATE_STR);
    
    for(j = 1; j <= dynlen(projDpList); j++){
      dynClear(dpeList);
      fwSystemOverviewUiFunc_makeProjDpeConnectList(projDpList[j], dpeList);

      dpConnect(workFuncProj, firstUpdate, dpeList);
    }
  }
  return 0;   
}

int fwSystemOverviewUiFunc_disconnectDp2Ui(string workFuncSys, string workFuncProj)
{
  dyn_string sysDpList, projDpList, dpeList;
  int i, j;
  ...
  for(i = 1; i <= dynlen(sysDpList); i++){
    fwSystemOverviewFunc_getProjDPList(sysDpList[i], projDpList);
    dpDisconnect(workFuncSys, sysDpList[i] + fwSystemOverviewFunc_CURR_STATE_STR);
    
    for(j = 1; j <= dynlen(projDpList); j++){
      dynClear(dpeList);
      fwSystemOverviewUiFunc_makeProjDpeConnectList(projDpList[j], dpeList);
      
      dpDisconnect(workProjFunc, dpeList);
    }
  }
  return 0;   
}

void fwSystemOverviewUiFunc_makeProjDpeConnectList(string projDp, dyn_string &dpeList)
{
...
void fwSystemOverviewUiFunc_updateSysStateInTree(shape tree, string sysDp, string command)
{
  int sysState;
  dyn_string projDpList;
  
  fwSystemOverviewFunc_getSysState(sysDp, sysState);

  if(command == "SYSTEMVIEW" || command == "PROCESSVIEW" || command == "MAPPINGVIEW"){
    tree.setText(sysDp, fwSystemOverviewUiFunc_STATE_COL, fwSystemOverviewFunc_getStrFromState(sysState));
  }
    ...
    for(int i = 1; i <=dynlen(projDpList); i++){
      tree.setText(tree.parent(projDpList[i]), fwSystemOverviewUiFunc_STATE_COL, fwSystemOverviewFunc_getStrFromState(sysState));
    }
  }
}
...
int fwSystemOverviewUiFunc_FSMSTATE_COL       = 1;    //Already defined
int fwSystemOverviewUiFunc_NODETYPE_COL       = 2;
...
//  tree.setIcon(treeNodeName, fwSystemOverviewUiFunc_FSMNAME_COL, PROJ_PATH + "pictures/fwSystemOverview/system.bmp");
  ...
  tree.setText(treeNodeName, fwSystemOverviewUiFunc_FSMSTATE_COL, state);
