...
Constraints:
	. global variable: the following variables are global to the script
		...
		. g_DistConnected: is the local PVSS00dist manager connected
	. constant:
...
localSystemIds contains the manager number of the local PVSS00dist. No value means that the PVSS00dist 
is not connected, therefore all the _unDistributedControl have to be set to false because there is no connection to the 
remote systems. 

remoteSystemIds contains the list of remote PVSS00dist manager number connected and ready to the local PVSS00dist, equivalent to 
remote system name because the PVSS00dist manager number must be unique within the whole distributed system. 
