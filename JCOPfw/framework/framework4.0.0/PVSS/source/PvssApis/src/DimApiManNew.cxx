...
{
public:
	DimManager(/*char *name*/char *file_name, char *dp_name);
...
//				serviceItem->setFirst(1);
	...
	char *getWriteDp() { return toWriteDP; };
	void *getWriteData() { return itsServData; };
	int getWriteSize() { return itsServCurrSize; };
...
public :
	...
	void setWriteInvFlag() 
		{ itsToWriteInvFlag = 1; itsManager->setFlag(this, 2);};
...
*/
	...
	CmndItem *findCmndState(int state);
private :
	char *itsDpName;
	char *itsDpItemNames;
	char *itsDimName;
	...
	int check_data(int type, char *dp, int size, char * &codes);
	...
	void setupDimDps(char *serv_name, int n, char *dps, int dpslen,
		DIM_TYPES *types);
};
...
int DimObject::check_data(int type, char *dp, int size, char * &codes)
{
	int ret_size = 0;
	int n, ret;
	static char code;
...
char *data_ptr, **ptr_arr, format[128]; 
	...
	else
	{
		if(!isCmnd())
		{
			...
			else
			{
				...
				CmndItem *item;
				void *addr;
				int size1, id, found = 0;

				if(itsRPCCommand)
				{
					do
					{
						item = (CmndItem *)itsRPCCommand->findCmndState(-1);
						if(item)
						{
							found = 1;
...
//	if(invalid)
//		qf = 0;
	if((invalid != itsInvalid) || (qf != itsQuality))
	{
		for(i = 0; i < itsNTypes; i++)
		{
			dpPtr = itsDpItemPtrs[i];
			...
			qf &= 0xf;
//			setDPAttr(dp_ptr,"",":_original.._userbit1",invalid);
//			setDPAttr(dp_ptr,"",":_original.._userbit8",qf);
			if(invalid || qf)
//				setDPAttr(dp_ptr,"",":_original.._aut_inv",(bit) 1);
				dpPtr->setInvalid(1);
			else
				dpPtr->setInvalid(0);
//				setDPAttr(dp_ptr,"",":_original.._aut_inv",(bit) 0);
//			dp_ptr += strlen(dp_ptr)+1;
			setWriteInvFlag();
		}
		itsInvalid = invalid;
		itsQuality = qf;
...
DpIdValueList list;
	...
	{
		dpPtr = itsDpItemPtrs[i];
		addDPAttr(list,dpPtr->getWriteDp(),"",":_original.._userbit8",
			dpPtr->getBit(0));
		addDPAttr(list,dpPtr->getWriteDp(),"",":_original.._userbit7",
			dpPtr->getBit(1));
		addDPAttr(list,dpPtr->getWriteDp(),"",":_original.._userbit6",
			dpPtr->getBit(2));
		addDPAttr(list,dpPtr->getWriteDp(),"",":_original.._userbit5",
			dpPtr->getBit(3));
		addDPAttr(list,dpPtr->getWriteDp(),"",":_original.._userbit1",
			dpPtr->getBit(7));
		addDPAttr(list,dpPtr->getWriteDp(),"",":_original.._aut_inv",
			dpPtr->getInvalid());
	}
//	sendDPAttrs(list);
	return 1;
...
void DimObject::commandHandler()
{
	...
	if(itsRPCCommand)
	{
		...
		else item->setState(1);
	}
	handleCommand(addr, size, id);
}

CmndItem *DimObject::findCmndState(int state)
{
	CmndItem *item;
	...
	do
	{
		if(item->getState() == state)
			return item;
		item = (CmndItem *)cmndList.getNext();
	}while(item);
	return (CmndItem *)0;
}
...
void DimObject::handleCommand(void *addr, int size, int id)
{
	...
	char *name_ptr, *ptr, format[128], *code_ptr;
	int i, ret_size, tout;
...
//		}
		...
		item = findCmndState(1);
		if(item)
		{
			item->setState(-1);
			item1 = (CmndItem *)cmndList.getNext();
			if(item1)
			{
				item1->setState(1);
				item1->getCmndData(addr, size, id);
				handleCommand(addr, size, id);
			}
			item1 = (CmndItem *)cmndList.getHead();
			if(item != item1)
				itsRPCCommand->cmndList.remove(item1);
		}

//		item = (CmndItem *)cmndList.getHead();
//		if(item)
//		{
//			item->getCmndData(addr, size, id);
//			handleCommand(addr, size, id);
//			itsRPCCommand->cmndList.remove(item);
//		}
	}
...
{
	char *itemName, dpname[128];
 	char *name_ptr;
	int i;
	char format[512], *formatp, *ptr;
...
DimManager::DimManager(char *file_name, char *dp_name) : 
	ApiManager()
	/*,
	DimCommand(name,"C")
	*/
...
int DimManager::processDimLine(char *str, int dim_type, int config)
{
  ...
  char *ptr;
  int index;
  char *dpe, *dpe1, serv_name[132], serv_name1[132], nolink[64];
  int timeout, stamped, update, dpslen, dpslen1, merge;
  char *dpsptr, *dpsptr1, format[512], hash_index[32];
  DpIdentifier dp;
...
void DimObject::setupDimDps(char *serv_name, int n, char *dimdps,
		int dpslen, DIM_TYPES *dimtypes)
{
	...
	if(!itsDpItemPtrs)
	{
		...
		for(i = 0; i < itsNTypes; i++)
		{
			itsDpItemPtrs[i] = new DimDpItem(name_ptr);
			name_ptr += strlen(name_ptr)+1;
		}
	}
#ifdef DEBUG
		...
		int timeout, int dimstamped, char *nolink, int update)
{
	...
	setupDimDps(serv_name, n, dimdps, dpslen, dimtypes);
	itsTime = timeout;
...
//	sprintf(name,"PVSS_SYS%d:DIMHandler",sysid);
	DimManager *dimManager = new DimManager(file_name, dp_name);
