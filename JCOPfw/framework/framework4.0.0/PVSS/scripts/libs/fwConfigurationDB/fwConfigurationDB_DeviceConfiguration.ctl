...
@date   August 2008
*/
global string _fwConfigurationDB_fileVersion_fwConfigurationDB_DeviceConfiguration_ctl="3.3.52";
...
/////////
    ...
    _fwConfigurationDB_startCommand(insertElementSql, g_fwConfigurationDB_DBConnection,
                                    insertElementCmd, exceptionInfo);
    if (dynlen(exceptionInfo)) {rdbRollbackTransaction(g_fwConfigurationDB_DBConnection);return;}
...
//DebugN("Storing properties for device",deviceListObject[fwConfigurationDB_DLO_DPNAME][i]);
	...
	// loop over all elements in a device
	    ...
	    //DebugN("Save prop:",dpe);
            ...
            nProps--;
            propParams[":IPROP_ID"] =   ipropid;
            propParams[":CITEM_ID"] =   deviceListObject[fwConfigurationDB_DLO_CITEM_ID][i];


            propParams[":DEVELEM_ID"] =   elementIds[j];
	    ...
	    if (storeValues) {
        	...
        	propParams[":VALUE"] = "\'"+ svalue+"\'";
            } else {
        	propParams[":VALUE"] = "NULL";
            }

            _fwConfigurationDB_bindExecuteCommand(insertElementCmd, propParams, exceptionInfo);
            if (dynlen(exceptionInfo)) {rdbRollbackTransaction(g_fwConfigurationDB_DBConnection);return;}
        };
        deviceListObject[fwConfigurationDB_DLO_PROPIDS][i]=devIpropIds;
    }

    _fwConfigurationDB_finishCommand(insertElementCmd, exceptionInfo);
    if (dynlen(exceptionInfo)) {rdbRollbackTransaction(g_fwConfigurationDB_DBConnection);return;}
...
//    string propidsListAsSql=_fwConfigurationDB_listToSQLString("IPROP_ID", propIds);
    ...
    for (int i=1;i<=max;i++) {
        ...
        dyn_mixed options=strsplit(aRecords[7][i],"|");
	...
	int alertType=aRecords[2][i];
        ...
        case DPCONFIG_SUM_ALERT:
    		...
    		//DP list specified by pattern
                ...
                sumAlertByPatternData[7][idx]=makeDynString(substr(aRecords[7][i],8)); // dp pattern
