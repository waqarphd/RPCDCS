...
global string _fwConfigurationDB_fileVersion_fwConfigurationDB_Hierarchies_ctl="3.3.52";
...
void _fwConfigurationDB_getReferencesFromDB( dyn_dyn_mixed &deviceListObject, string configurationName, dyn_string &exceptionInfo)
{
    ...
    if (dynlen(aRecords)==0) {dyn_mixed empty;aRecords[1]=empty;};
	...
	int options=fwConfigurationDB_deviceConfig_ALLDEVPROPS)
{
    ...
    if (CONFIG_APPLY_VALUES){
        ...
        DebugN("performing dpset for "+dynlen(valDpes)+" values");
        int rc=dpSetWait(valDpes,values);
        DebugN("dpset result",rc);
    }
...
*/
   if (fwConfigurationDB_progress(fwConfigurationDB_OPER_SaveDeviceProperties,"Preparing...",  10.0, exceptionInfo)) return;
	...
	dynClear(configurationData);
	_fwConfigurationDB_extractAlertConfiguration(dpes,ipropIds,
	...
	dynClear(configurationData);
	_fwConfigurationDB_extractArchivingConfiguration(dpes,ipropIds, configurationData, exceptionInfo);
	if (dynlen(exceptionInfo)) {rdbRollbackTransaction(g_fwConfigurationDB_DBConnection);return;}
	...
	dynClear(configurationData);
	_fwConfigurationDB_extractDpFunctionConfiguration(dpes,ipropIds, configurationData, exceptionInfo);
	if (dynlen(exceptionInfo)) {rdbRollbackTransaction(g_fwConfigurationDB_DBConnection);return;}
	...
	dynClear(configurationData);
	_fwConfigurationDB_extractConversionConfiguration(dpes,ipropIds, configurationData, exceptionInfo);
	...
	dynClear(configurationData);
	_fwConfigurationDB_extractPvRangeConfiguration(dpes,ipropIds, configurationData, exceptionInfo);
	...
	dynClear(configurationData);
	_fwConfigurationDB_extractSmoothingConfiguration(dpes,ipropIds, configurationData, exceptionInfo);
