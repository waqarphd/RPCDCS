/**
  ...
  @return nothing
*/
void invokedAESUserFunc( string shapeName, int screenType, int tabType, int row, int column, string value )
{
  bool isGranted;
  int position;
...
//  DebugN("invokedAESUserFunc() - shape=" + shapeName + " column=" + column + " row=" + row );
  ...
  else if(columnName == fwAlarmHandlingScreen_COLUMN_ACKNOWLEDGE)  
  {
    ...
    aesuser_doAcknowledge(screenType, tabType, row, column, value);
  }
}

aesuser_doAcknowledge(string screenType, int tabType, int row, string column, string value )
{
  ...
  res=aes_getDpidFromTable( row, dpid );
  if( res != 0 )
  {
    return;
  }
  ...
  {
    aes_changedAcknowledge( AES_CHANGED_ACKSINGLE, tabType, row );
  }
