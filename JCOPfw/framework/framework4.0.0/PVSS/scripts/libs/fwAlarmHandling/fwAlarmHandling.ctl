...
const int fwAlarmHanlding_HELP_DEFAULT               = 5;
...
//DebugN(systemNameFilters, dpNameFilters, dpAliasFilters, dpTypeFilters);
	...
	for(i=1; i<=numberOfSystems; i++)
	{
		...
		for(j=1; j<=numberOfTypes; j++)
		{
			...
			{
				searchResult = dpNames(systemNameFilters[i] + dpNameFilters[k], dpTypeFilters[j]);
//DebugN("DP NAME FILTER", systemNameFilters[i] + dpNameFilters[k], dpTypeFilters[j], searchResult);
				if(k==1)
					dps = searchResult;
				else
...
@param y			input, Optional parameter - default value 0.  Y position of the new display
*/
fwAlarmHandling_openScreenWithFilter(dyn_dyn_anytype filter, bool bAsNewModule, bool bStayOnTopOrModal, string sModuleName, string sPanelName, dyn_string &exceptionInfo, unsigned x = 0, unsigned y = 0)
{
...
_fwAlarmHandling_createPlotDp(string plotName)
{
  ...
  plotData[fwTrending_PLOT_OBJECT_CURVE_TYPES] = makeDynInt(fwTrending_PLOT_TYPE_LINEAR, fwTrending_PLOT_TYPE_LINEAR,
                                                            fwTrending_PLOT_TYPE_LINEAR, fwTrending_PLOT_TYPE_LINEAR,
                                                            fwTrending_PLOT_TYPE_LINEAR, fwTrending_PLOT_TYPE_LINEAR,
                                                            fwTrending_PLOT_TYPE_LINEAR, fwTrending_PLOT_TYPE_LINEAR);
