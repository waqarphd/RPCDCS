...
fwArchive_set(string dpe, string archiveClassName, int archiveType, int smoothProcedure,
				float deadband, float timeInterval, dyn_string &exceptionInfo, bool checkClass = TRUE) 
{
	fwArchive_setMany(makeDynString(dpe), makeDynString(archiveClassName), makeDynInt(archiveType), makeDynInt(smoothProcedure),
...
{ 
	...
	switch(configType)
	{
		case DPCONFIG_DB_ARCHIVEINFO:
			...
			_fwSmoothing_getParameters(dpe, TRUE, smoothProcedure, deadband, timeInterval, exceptionInfo);
