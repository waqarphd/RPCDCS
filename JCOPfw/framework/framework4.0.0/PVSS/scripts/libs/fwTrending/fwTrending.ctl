...
const int fwTrending_PLOT_OBJECT_EXT_MIN_FOR_LOG = 30;
const int fwTrending_PLOT_OBJECT_EXT_MAX_PERCENTAGE_FOR_LOG = 31;
const int fwTrending_PLOT_OBJECT_EXT_UNITS_X = 32;
const int fwTrending_PLOT_OBJECT_EXT_UNITS = 33;
const int fwTrending_PLOT_OBJECT_EXT_TOOLTIPS_X = 34;
const int fwTrending_PLOT_OBJECT_EXT_TOOLTIPS = 35;
const int fwTrending_PLOT_OBJECT_EXT_MIN_MAX_RANGE_X = 36;
const int fwTrending_PLOT_OBJECT_EXT_MIN_MAX_RANGE = 37;
...
const int fwTrending_SIZE_PLOT_OBJECT = 37;
...
const int fwTrending_getPlotData_LEN_RETURN_DATA = 37;
...
void fwTrending_newPlot(string _plotName, string model = fwTrending_YT_PLOT_MODEL)
{
    ...
    data[fwTrending_PLOT_OBJECT_LEGEND_ON] = FALSE;
    ...
    data[fwTrending_PLOT_OBJECT_CURVES_HIDDEN] = makeDynBool();
    ...
    data[fwTrending_PLOT_OBJECT_MARKER_TYPE] = 0;
    data[fwTrending_PLOT_OBJECT_ACCESS_CONTROL_SAVE] = "";
	data[fwTrending_PLOT_OBJECT_CURVE_TYPES] = makeDynInt(fwTrending_PLOT_TYPE_POINTS, fwTrending_PLOT_TYPE_POINTS, fwTrending_PLOT_TYPE_POINTS, fwTrending_PLOT_TYPE_POINTS,
...
//DebugN(temp, texts);
	...
	_fwTrending_initGenericTrendControls(ref, sDpName, plotData[fwTrending_PLOT_OBJECT_GRID], plotData[fwTrending_PLOT_OBJECT_LEGEND_ON], exceptionInfo, bCallBack);
								...
								dyn_string curveTimeObj, bool bEnabledVisibility)
{
	...
	// if the visibility is enalbed then show the curveTime.
...
@param markerType		input, type of marker (default value = 7 - filled circle);
*/
...
//DebugN("curveLegend visible:", curveName);
...
// stop both trends
	standardTrendShape.trendStop();
//start/end 20/05/2008: Herve	logTrendShape.trendStop();
...
//start/end 20/05/2008: Herve	logTrendShape.timeInterval(timeRange);
	standardTrendShape.timeBegin = getCurrentTime()-standardTrendShape.timeInterval();
//start/end 20/05/2008: Herve	logTrendShape.timeBegin = getCurrentTime()-logTrendShape.timeInterval();
	
	//added to prevent problem with historical data not being drawn correctly
	delay(0,200);
...
//start/end 20/05/2008: Herve	logTrendShape.backCol = plotExtendedObject[fwTrending_PLOT_OBJECT_BACK_COLOR][1];
//start/end 20/05/2008: Herve	logTrendShape.foreCol = plotExtendedObject[fwTrending_PLOT_OBJECT_FORE_COLOR][1];

	standardTrendShape.scrollPercent = 5;
//start/end 20/05/2008: Herve	logTrendShape.scrollPercent = 5;
...
*/
fwTrending_showStandardTrend(bool visibility, string standardStr, string logStr)
{
...
// stop the active trend
	activeTrendShape.trendStop();
...
{
	dyn_string dsTemp, dsCurvesHidden, split, dsLeg, exceptionInfo;
	int len, i, iTemp;
...
//start/end 20/05/2008: Herve		logTrendShape.timeInterval(range);
		...
		standardTrendShape.timeBegin() = getCurrentTime() - range;
...
fwTrending_pageExportTrend(dyn_string dsRef, string templateParameters)
{
	...
	dyn_string dsTemp, dsDpe, dsLegend, exceptionInfo;
	string isRunning;
	dyn_float df;
	dyn_string plotShapes, plotData, saveOptions;
...
// append the tStart to the list
		dynAppend(dstStart, tempdstStart);
// append the tEnd to the list
		dynAppend(dstEnd, tempdstEnd);
// append the temp list dpe to dpe list
		dynAppend(dsDpe, dsTemp);
// get the curve legend and append it to dsLegend
		temp = plotData[fwTrending_PLOT_OBJECT_LEGENDS];
		fwTrending_convertStringToDyn(temp, dsTemp, exceptionInfo);
		dynAppend(dsLegend, dsTemp);
	...
	if(noArch) {
		ChildPanelOnCentralReturn("fwGeneral/fwOkCancel.pnl", "Export data to CSV",
															makeDynString("$text:Some of the data point elements do not have\n"
																								...
																								+ "exported correctly.  Do you wish to continue?"), df, saveOptions);
		if(saveOptions[1] != "ok")
			return;
	}
...
//DebugN("expressionList",expressionList);

	_fwTrending_splitParameters(givenParametersString, givenParameters);

//DebugN("given Parameters", givenParameters);
	...
	{
		if(dynContains(givenParameters[1], parametersNeeded[i]) <= 0)
...
*/
fwTrending_openPlotDisplay(string sPlotDpName, string sPanelFileName, bool bAsNewModule, bool bStayOnTopOrModal, string sModuleName, string sPanelName, unsigned x = 0, unsigned y = 0, string sReziseMode="")
{
...
_fwTrending_convertPlotDataToDollars(dyn_dyn_string plotData, dyn_string &dollarParameters, dyn_string &exceptionInfo)
{
	...
	dynAppend(dollarParameters, "$dsCurveLegend:" + stringValue);
	...
	dynAppend(dollarParameters, "$dsCurveVisible:" + stringValue);
	fwTrending_convertDynToString(plotData[fwTrending_PLOT_OBJECT_AXII], stringValue, exceptionInfo);
	dynAppend(dollarParameters, "$dsCurveScaleVisible:" + stringValue);
	...
	dynAppend(dollarParameters, "$sForeColor:" + plotData[fwTrending_PLOT_OBJECT_FORE_COLOR][1]);
	dynAppend(dollarParameters, "$bShowGrid:" + plotData[fwTrending_PLOT_OBJECT_GRID][1]);
	dynAppend(dollarParameters, "$bShowLegend:" + plotData[fwTrending_PLOT_OBJECT_LEGEND_ON][1]);
	dynAppend(dollarParameters, "$iMarkerType:" + plotData[fwTrending_PLOT_OBJECT_MARKER_TYPE][1]);
	dynAppend(dollarParameters, "$sTimeRange:" + plotData[fwTrending_PLOT_OBJECT_TIME_RANGE][1]);
}
