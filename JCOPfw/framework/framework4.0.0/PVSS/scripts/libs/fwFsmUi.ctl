...
int fwFsmUi_connectModeBits(string callback, string domain, string obj, int no_wait = 1, string part = "")
{
	int ret;

	ret = fwUi_connectModeBits(callback, domain, obj, no_wait, part);
	return ret;
}
...
//DebugTN("fwUi_show_mode Returning!",myDomain, myObj);
	...
	if(!FSMBusy)
		fwUi_getLocalObjStateActions(myDomain, myObj, State, actions, visis);
	else
	{
...
//DebugTN("fwUi_show_mode",myDomain, myObj, State, actions, visis);
	AccessControlOperator = fwFsmUi_getOperatorAccess(myDomain, myObj);
	AccessControlExpert = fwFsmUi_getExpertAccess(myDomain, myObj);
...
fwFsmUi_connectLock(string domain, string obj, bool isCU = true, string part = "")
{
	myDomain = domain;
	myObj = obj;
//DebugN("ConnectingModeBits", domain, obj);
	if (isCU) 
		fwFsmUi_connectModeBits("_fwFsmUi_show_it", domain, obj);
	else 
		fwFsmUi_connectModeBits("_fwFsmUi_dev_show_it", domain, obj, 1, part);
}
...
*/
			if (getBit(statusBits,FwIncompleteBit))
