...
//DebugTN("appendTreeNodes", treeIndex, levels, parent, node, sys);
...
//DebugN("*******", node, children, levels);
	for(i = 1; i <= dynlen(children); i++)
	{
   if( (dynlen(FwTreeTopNodes)) && (FwTreeModes[treeIndex] == "operator"))
   {
     if((node == "FSM") && (!dynContains(FwTreeTopNodes, children[i])))
       continue;
   }
		if(children[i] != node)
		{
