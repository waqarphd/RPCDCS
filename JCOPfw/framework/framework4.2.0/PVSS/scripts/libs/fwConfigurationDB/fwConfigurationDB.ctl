...
global string _fwConfigurationDB_fileVersion_fwConfigurationDB_ctl="3.3.63";
 ...
 * @ingroup Constants
*/
const float      fwConfigurationDB_version = 3.0;

/** Minimal version of the DB schema required for operation
 * see also @ref fwConfigurationDB_version
 * @ingroup Constants
*/
const float      fwConfigurationDB_minimalDBSchemaVersion=2.06;


const float     fwConfigurationDB_minimalRDBAccessVersion=0.60;
...
synchronized void  fwConfigurationDB_initialize(string setupName, dyn_string &exceptionInfo)
{
	...
	fwException_raise(exceptionInfo,"ERROR","RDB Access functions not available\n"+
			"Please install the CtrlRDBAccess PVSS patch!","");
	DebugN("ERROR: CtrlRDBAccess PVSS patch probably not installed");
	DebugN("It is available from http://cern.ch/itcobe/Projects/Framework/Download/Components/Configuration/");
	return;
    }
    ...
    if (g_fwConfigurationDB_DBConnectionOpen) {
        ...
        //dbCloseConnection(g_fwConfigurationDB_DBConnection);
    }
