...
void fwConfigurationDB_updateDBSchema(dbConnection dbCon, dyn_string &exceptionInfo)
{
    ...
    _fwConfigurationDB_EnsureSequencesAreSane(exceptionInfo);
    if (dynlen(exceptionInfo)) return;
                                        ...
                                        dyn_string &exceptionInfo)
{
    ...
    if (dpExists(dpName)){
	fwException_raise(exceptionInfo,"ERROR", "Database connection "+name+" already exists","");
	return "";
    }


    int rc=dpCreate(dpName,"_FwDBConnection");
    ...
    fwConfigurationDB_modifyDBConnection(dbConnectionName, description, dbType, dbName, userName,
					password, connectString,exceptionInfo);
    return dpName;
...
{
   _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "ITEMS_ID_SEQ", "ITEMS", "ID",exceptionInfo);
    if (dynlen(exceptionInfo)) return;

    _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "RECIPE_DATA_PROPID_SEQ", "RECIPE_DATA", "PROPID",exceptionInfo);
    if (dynlen(exceptionInfo)) return;

    _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "RECIPES_RVER_SEQ", "RECIPE_VERSIONS", "RVER",exceptionInfo);
    if (dynlen(exceptionInfo)) return;

    _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "RECIPES_TAGID_SEQ", "RECIPES", "TAGID",exceptionInfo);
    if (dynlen(exceptionInfo)) return;

    _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "IPROP_ID_SEQ", "CONFIG_ITEM_PROPERTIES", "IPROP_ID",exceptionInfo);
    if (dynlen(exceptionInfo)) return;

    _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "CONF_ID_SEQ", "CONFIG_DESC", "CONF_ID",exceptionInfo);
    if (dynlen(exceptionInfo)) return;

    _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "DPT_ID_SEQ", "DEVICE_TYPES", "DPT_ID",exceptionInfo);
    if (dynlen(exceptionInfo)) return;

    _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "DEVELEM_ID_SEQ", "DEVICE_ELEMENTS", "DEVELEM_ID",exceptionInfo);
    if (dynlen(exceptionInfo)) return;

    _fwConfigurationDB_checkSequenceSane(g_fwConfigurationDB_DBConnection, "CITEM_ID_SEQ", "CONFIG_ITEMS", "CITEM_ID",exceptionInfo);
    if (dynlen(exceptionInfo)) return;
...
void _fwConfigurationDB_DBCheckCreateSystemNode(string hierarchyType, dyn_string &exceptionInfo, string system="")
{
    ...
    dyn_int diSeq;
    _fwConfigurationDB_getBatchOfSequenceNumbers(g_fwConfigurationDB_DBConnection, "ITEMS_ID_SEQ", 1, diSeq, exceptionInfo);
    if (dynlen(exceptionInfo)) return;
    string nextId=diSeq[1];
    string insertRootNodeSql="INSERT INTO ITEMS (HVER,ID,PARENT,NAME,TYPE,DETAIL,DESCRIPTION,DPID,DPNAME,DPT_ID) "+
        "VALUES ("+g_fwConfigurationDB_DBHierarchyIDs[hierarchyType]+","+nextId+",NULL,"+
        "\'"+systemName+"\',\'SYSTEM\',\'SYSTEM\',NULL,NULL,\'"+systemName+"\',NULL)";
    fwConfigurationDB_executeSqlSimple(insertRootNodeSql, g_fwConfigurationDB_DBConnection,exceptionInfo);
    if (dynlen(exceptionInfo)) return;

//    _fwConfigurationDB_updateHierarchyView(g_fwConfigurationDB_DBConnection, exceptionInfo);
//    if (dynlen(exceptionInfo)) return;

DebugN("created root node in DB for "+systemName+" hierarchy:"+hierarchyType);
}


/** Checks if the node called "/" for logical hierarchy is already present...
*/
void _fwConfigurationDB_checkCreateCommonRootNodeForLogicalHierarchy(dyn_string &exceptionInfo)
{
    string queryRootNodeSql="SELECT ID FROM ITEMS WHERE NAME=\'/\'"+
        " AND HVER="+g_fwConfigurationDB_DBHierarchyIDs[fwDevice_LOGICAL];
    dyn_dyn_mixed aRootNode;
    _fwConfigurationDB_executeDBQuery(queryRootNodeSql,g_fwConfigurationDB_DBConnection, aRootNode, exceptionInfo);
    if (dynlen(exceptionInfo)) return;
    if (dynlen(aRootNode)>0) return; // OK. it exists...
    ...
    string nextId=diSeq[1];
    string insertRootNodeSql="INSERT INTO ITEMS (HVER,ID,PARENT,NAME,TYPE,DETAIL,DESCRIPTION,DPID,DPNAME,DPT_ID) "+
        "VALUES ("+g_fwConfigurationDB_DBHierarchyIDs[fwDevice_LOGICAL]+","+nextId+",NULL,"+
        "\'/\',\'SYSTEM\',\'SYSTEM\','Top of unified LOGICAL hierarchy',NULL,NULL,NULL)";
    fwConfigurationDB_executeSqlSimple(insertRootNodeSql, g_fwConfigurationDB_DBConnection,exceptionInfo);
    if (dynlen(exceptionInfo)) return;
...
DebugN("created root node in DB for unified LOGICAL hierarchy");
