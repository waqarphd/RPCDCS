...
@param configExists		output, TRUE is addressing is active, else FALSE
@param exceptionInfo	output, details of any exceptions are returned here
*/
...
//				DebugN("driverNum_ini= "+driverNum_ini+" pollGroup_ini= "+pollGroup_ini);
        ...
        setMultiValue("s7ConnNames","text",reference1_ini,
                    	...
                    	"cmbPollGroup", "text",pollGroup_ini,
...
@param default			output, encoded S7 address
*/  
...
@param default			output, transformation type: i.e.: "701 --> int 16"
*/  
...
@param pa			output, correct address S7 type. i.e.: "DB100.DBX200.1"
*/
...
@param directionMode			input, direction mode (Output, Input, Input/Output)
...
_fwPeriphAddressS7_setIOMode(int io, int im, string sSystemName)
{
  ...
  if (shapeExists("frmPollGroup"))
 		{
 			alreadyVisible = cmbPollGroup.visible;
	  	frmPollGroup.visible = (io==4 || io==7);
	  	txtPollGroup.visible = (io==4 || io==7);
	  	cmbPollGroup.visible = (io==4 || io==7);
	  	cmdPollGroup.visible = (io==4 || io==7) & (sSystemName == getSystemName());
	  	
	  	if (cmbPollGroup.visible)
	  		{
  				...
  				cmbPollGroup.items = dsPlc;
  				if (!alreadyVisible) cmbPollGroup.selectedPos =1;
	  		}
  	}
}
