...
@param dpes						input, list of data point elements to read
...
@param values					input, the list of original values for each dpe are passed here
...
fwOriginal_set(dyn_string dpe, anytype value, dyn_bool infoBits, dyn_string &exceptionInfo, string mode = "VALUE_VARIABLE_BITS_USER_BITS")
{
	...
	fwOriginal_setMany(makeDynString(dpe), makeDynAnytype(value), infoBitArray, exceptionInfo, mode);
}
