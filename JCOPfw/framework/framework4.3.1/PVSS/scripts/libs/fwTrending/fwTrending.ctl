...
@par Modification History
  ...
  16/06/2011: Marco
    - fix for #83252: Histogram: values not plotted if dpe passed as template param without sys name. 
    in the function _fwTrending_updateHistogram(), if no system name was passed as template {} parameter, add the local sys name
    
  29/01/2011: Marco
    - fix for #77214, ENS-2447: assign dpe legend name to the legend of the alarm limits.
    - fix for #77700: new functions fwTrending_getPageTreeTagNames(), fwTrending_getPlotTreeTagNames()
  ...
  02/12/2010: Marco
    - fix for #75930: After upgrading existing trending to fwTrending 3.3.1, 
      the date on the X axis was shown twice on 2 lines. Now the default value is applied.
    
  12/11/2010: Marco
    - fix for #75191: Customizable default plot setting. Modified fwTrending_newPlot(): it checks now if
  ...
  11/11/2010: Marco
    - fix for #71141: Uncaught exception during install of trending tool. 
  ...
  27/10/2010: Marco
    - fix for #74445 (Hide/show legend also on trend pages). Modified function fwTrending_controlBarOnOff()
  ...
  04/08/2010: Marco
    - fix for ENS-1045 (log scale): when activating the Log scale, now the function fwTrending_showStandardTrend()
  ...
  30/07/2010: Marco
    - fix for #68678 (Plot alarm limits): the limits of the alarm of 
  ...
  30/06/2010: Marco
    ...
    - fix for #69566: for each curve, the value format on the scale can be customized. 
    ...
    - fix for #69424: for each curve, value format on the legend can be customized. 
    ...
    - fix for #68074: date format for the X axis can be customized. 
  ...
  26/05/2010: Marco
    - fix for #55166: curves line can be modified in width and style. 
      This info is on fwTrending_PLOT_OBJECT_CURVE_STYLE.
      On the panels, this parameter is appended to the param $iMarkerType.
    - fix for #67907: fonts can be modified. This info is on fwTrending_PLOT_OBJECT_DEFAULT_FONT.
      On the panels, this parameter is appended to the param $sForeColor.
    - fix for #55167,	#66698: curves can be linked to each other. Each curve has a new property: 
  ...
  29/04/2010: Marco
    - fix for #66064,	#66488: a new function fwTrending_controlBarOnOff() toggles the control menu bar and 
      the captions bar.
      If the bar is hidden, the trending area is expanded. 
      The captions can be hidden, the trending area is expanded.
    - fix for #35454: scale position: the trending can now show the scale of a curve at the left or at the right. 
  ...
  06/04/2010: Marco
    - fix for #65392: a new function _fwTrending_gotoNow() shows the last value using a different 
      procedure depending on the PVSS version. Replaced all the .gotoNow() with _fwTrending_gotoNow().
      
  25/03/2010: Marco
    - fix for #40368: added a check for empty values of datapoints to _fwTrending_updateHistogram(). 
      Now, if there is no value in the dpe to plot, the histogram is cleaned. 
    - fix for #55165: skip evaluation of parameters when calling _fwTrending_evaluateTemplate() 
      passing the string "parameterValues" as given parameter.
    - fix for #64959: _fwTrending_updateHistogram() could not load dpe from template "{}". Fixed.
      ...
      string sAxiiYFormat = fwTrending_DEFAULT_AXII_Y_FORMAT, bool bAlarmLimitsVisible = FALSE)
{
	...
	int i,j;
	float minForLog, maxPercentageForLog, currentMax, fConstantValue;
...
*/
    ...
    if(bConstantLine)
    {
      activeTrendShape.curveValue(curveName, fConstantValue,
...
// DebugN("value: "+value+ " fValue: "+fValue);
  return fValue;
}

bool _fwTrending_isConstantLineValue(string value)
{
//  DebugN("value: "+value+ " substr: "+ (substr(value, 0,1)=="#"));
  value = strltrim(strrtrim(value));
                             ...
                             string sCurveStyle=fwTrending_DEFAULT_CURVE_STYLE, bool clearLimits = FALSE)
{
  ...
  string alertStateBits, curveLegend, curveUnit;
