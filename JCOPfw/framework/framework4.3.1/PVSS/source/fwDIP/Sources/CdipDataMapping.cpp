...
m_alreadyReportedDataMappingProblem(false),
m_alreadyReportedTimestampProblem(false),
...
void CdipDataMapping::addValueValidityStatusToList(DpIdValueList &list){
	...
	///////////
	// FWDIP-79 Add a check for timestamps that are suspiciously old (more than 48 hours)
	TimeVar now;
	PVSSboolean timestampIsTooOld = false;
	if( (getMappedDpe().getMapping()->timeSecondsPVSS + DipApiResources::getStaleDipTimestampThresholdInSeconds() ) < now.getSeconds() ){
		timestampIsTooOld = true;
	}
	list.appendItem(getMappedDpe().getDIPusrBit4ID(), new BitVar(timestampIsTooOld));
	// end of FWDIP-79
	//////////

	// FWDIP-79 Also log actual invalidity details through userbits
	list.appendItem(getMappedDpe().getDIPusrBit5ID(), new BitVar(badTypeConversion));
	list.appendItem(getMappedDpe().getDIPusrBit6ID(), new BitVar(valueConverter == NULL));
}
	...
	if (valueConverter == NULL)
	{
		...
		*/
		...
		catch(BadTypeConversionException &e)
		{
			...
			*/
			// DIP-96
			if( (!m_alreadyReportedDataMappingProblem) 
				|| (::DipApiResources::isLogDataMappingErrorsMultipleTimes()) ){
			  m_alreadyReportedDataMappingProblem = true;
			  PVSSERROR("Cannot find converter for DIP address '"<< (const char *)fullDIPAddress<<"' (exception reason : " << e.reason() << ")");
			}
		...
		if (mostRecentDipQuality != DIP_QUALITY_UNINITIALIZED)
		{
			...
			if( (::DipApiResources::isAttemptToPublishCorrectionValuesForDipTimestampsInThePast()) && (correctionValueTimestamp != 0) && (mappedDpe.isDpeBeingArchived() == true) ){
				...
				try{
					...
					{
					    if( (!m_alreadyReportedTimestampProblem) 
					  	|| (::DipApiResources::isLogDataMappingErrorsMultipleTimes()) ){
					        m_alreadyReportedTimestampProblem = true;
