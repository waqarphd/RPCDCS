...
/// Maps to the PVSS int type
class CdipIntToPVSSFloat:public CdipToPVSSTypeConverter{
public:
	VariablePtr convert(const std::string & tag, DipData & data) const{
		DipInt value;
...
class CdipLongToPVSSFloat:public CdipToPVSSTypeConverter{
public:
	VariablePtr convert(const std::string & tag, DipData & data) const{
		DipLong value;
...
static CdipIntToPVSSFloat	dipIntToPVSSFloatConverter;
static CdipLongToPVSS		dipLongToPVSSConverter;
static CdipLongToPVSSFloat	dipLongToPVSSFloatConverter;
...
TMap::TMap(){
	if (dipToPvsstypeConverterList.size() == 0){
		...
		implicitDipToDpeType[TYPE_FLOAT_ARRAY] = DPELEMENT_DYNFLOAT;
		...
		dipToPvsstypeConverterList[TYPE_INT | (DPELEMENT_FLOAT * 1000)]				= new TConv(TYPE_INT,			DPELEMENT_FLOAT,	dipIntToPVSSFloatConverter);
		dipToPvsstypeConverterList[TYPE_LONG | (DPELEMENT_INT * 1000)]				= new TConv(TYPE_LONG,			DPELEMENT_INT,		dipLongToPVSSConverter);
		dipToPvsstypeConverterList[TYPE_LONG | (DPELEMENT_FLOAT * 1000)]			= new TConv(TYPE_LONG,			DPELEMENT_FLOAT,	dipLongToPVSSFloatConverter);
