...
int subpub_sort, subpub_buff, subpub_over;
string subpub_item, subpub_tagn, subpub_type;
string dpe_name, dpe_type;
dyn_string sub_name_all_0 = makeDynString();
dyn_string sub_item_all_0 = makeDynString();
dyn_string sub_tags_all_0 = makeDynString();
dyn_string sub_name_all_1 = makeDynString();
dyn_string sub_item_all_1 = makeDynString();
dyn_string sub_tags_all_1 = makeDynString();
dyn_string pub_sgl_name_all_0 = makeDynString();
dyn_string pub_sgl_item_all_0 = makeDynString();
dyn_int    pub_sgl_buff_all_0 = makeDynString();
dyn_string pub_sgl_name_all_1 = makeDynString();
dyn_string pub_sgl_item_all_1 = makeDynString();
dyn_int    pub_sgl_buff_all_1 = makeDynString();
dyn_dyn_anytype pub_str_all = makeDynAnytype();
int pub_str_0 = 0;
int pub_str_1 = 0;
int pub_tag_0 = 0;
int pub_tag_1 = 0;
  ...
  while ( entry = fwExternalApps_getNextActiveFromTree ( XmlTable , entry , depth , elems , sub_pub , thedata , exceptionInfo ) )
  {
    ...
    */
    ...
    for ( index = 1 ; index <= data_size ; ++index )
    {
      ...
      subpub_sort = thedata[index][fwExternalApps_DATA_SUBS_PUBL];
      subpub_buff = thedata[index][fwExternalApps_DATA_BUFF_MODE];
      subpub_over = (thedata[index][fwExternalApps_DATA_OVER_WRITE]?1:0);
      subpub_item = thedata[index][fwExternalApps_DATA_SUBPUB_NAME];
      subpub_tagn = thedata[index][fwExternalApps_DATA_PNT_TAGNAME];
      subpub_type = thedata[index][fwExternalApps_DATA_SUBPUB_TYPE];
      dpe_name = thedata[index][fwExternalApps_DATA_PNT_NAME]
               + thedata[index][fwExternalApps_DATA_PNT_ELEMENT];
      dpe_type = thedata[index][fwExternalApps_DATA_PNT_TYPNAME];
      ...
      if ( ( subpub_type != "single" ) && ( subpub_type != "multiple" ) )
      {
        fwException_raise(exceptionInfo, "WARNING", "Subscr/Public '"+subpub_item+"' has invalid Type '"+subpub_type+"'", "");
        continue;
      }
      
      if ( ( subpub_sort != fwExternalApps_DATA_TYPE_SUBSCR ) && ( subpub_sort != fwExternalApps_DATA_TYPE_PUBLIC ) )
      {
        fwException_raise(exceptionInfo, "WARNING", "Subscr/Public '"+subpub_item+"' Must be a Subscr or Public! '", "");
        continue;
      }

      if ( subpub_sort == fwExternalApps_DATA_TYPE_SUBSCR )
      {
        if ( subpub_over == 0 )
        {
           dynAppend ( sub_name_all_0 , dpe_name );
           dynAppend ( sub_item_all_0 , subpub_item );
           dynAppend ( sub_tags_all_0 , ( subpub_type == "single" ? "" : subpub_tagn ) );
        }
        else
        {
           dynAppend ( sub_name_all_1 , dpe_name );
           dynAppend ( sub_item_all_1 , subpub_item );
           dynAppend ( sub_tags_all_1 , ( subpub_type == "single" ? "" : subpub_tagn ) );
        }
      ...
      {
        if ( subpub_type == "single" )
        {
          if ( subpub_over == 0 )
          {
            dynAppend ( pub_sgl_name_all_0 , dpe_name );
            dynAppend ( pub_sgl_item_all_0 , subpub_item );
            dynAppend ( pub_sgl_buff_all_0 , subpub_buff );
          }
          else
          {
            dynAppend ( pub_sgl_name_all_1 , dpe_name );
            dynAppend ( pub_sgl_item_all_1 , subpub_item );
            dynAppend ( pub_sgl_buff_all_1 , subpub_buff );
          }
        }
        else
        {
          for ( idx = 1 ; idx <= dynlen(pub_str_all) ; ++idx )
          {
            if ( subpub_item == pub_str_all[idx][1] ) break;
          }
          if ( idx > dynlen(pub_str_all) )
          {
            if ( subpub_over == 0 ) ++pub_str_0; else ++pub_str_1;

            dynAppend ( pub_str_all , makeDynAnytype(subpub_item,subpub_buff,subpub_over,makeDynString(),makeDynString()) );
          }
          else
          {
            if ( subpub_buff != pub_str_all[idx][2] )
            {
              fwException_raise(exceptionInfo, "WARNING", "Publication tag '"+subpub_item+"' '"+subpub_tagn
                            +"' has different buff-mode '"+subpub_buff+"' vs '"+pub_str_all[idx][2]+"'", "");
              continue;
            }
            if ( subpub_over != pub_str_all[idx][3] )
            {
              fwException_raise(exceptionInfo, "WARNING", "Publication tag '"+subpub_item+"' '"+subpub_tagn
                            +"' has different overwrite-mode '"+subpub_over+"' vs '"+pub_str_all[idx][3]+"'", "");
              continue;
            }
          }
          
          if ( pub_str_all[idx][3] == 0 ) ++pub_tag_0; else ++pub_tag_1;
              
          dynAppend(pub_str_all[idx][4],dpe_name);
          dynAppend(pub_str_all[idx][5],subpub_tagn);
        }
      }
  ...
  if ( tracing > 0 ) DebugN("Number of Subscriptions [Overwrite=False] are '"+dynlen(sub_name_all_0)+"'");
  
  fwDIP_subscribeMany ( sub_name_all_0 , configName , sub_item_all_0 , sub_tags_all_0 ,
                        exceptionInfo , FALSE );

  if ( tracing > 0 ) DebugN("Number of Subscriptions [Overwrite=True]  are '"+dynlen(sub_name_all_1)+"'");

  fwDIP_subscribeMany ( sub_name_all_1 , configName , sub_item_all_1 , sub_tags_all_1 ,
                        exceptionInfo , TRUE );
  
  if ( tracing > 0 ) DebugN("Number of Primitive(single) Pubs [O/W=F]  are '"+dynlen(pub_sgl_name_all_0)+"'");
  for ( idx = 1 ; idx <= dynlen(pub_sgl_name_all_0) ; ++idx )
  {
    fwDIP_publishPrimitive ( pub_sgl_item_all_0[idx] , pub_sgl_name_all_0[idx] , pub_sgl_buff_all_0[idx] , configName , strException , FALSE );
  }
  
  if ( tracing > 0 ) DebugN("Number of Primitive(single) Pubs [O/W=T]  are '"+dynlen(pub_sgl_name_all_1)+"'");
  for ( idx = 1 ; idx <= dynlen(pub_sgl_name_all_1) ; ++idx )
  {
    // DebugN ( "Publication Primitive" , ub_sgl_item_all_1[idx] , pub_sgl_name_all_1[idx] , pub_sgl_buff_all_1[idx] , configName , strException , TRUE );
    
    fwDIP_publishPrimitive ( pub_sgl_item_all_1[idx] , pub_sgl_name_all_1[idx] , pub_sgl_buff_all_1[idx] , configName , strException , TRUE );
  }
  
  if ( tracing > 0 ) DebugN("Nbr of Structured(multiple) Pubs [O/W=F]  are '"+pub_str_0+"' [tags="+pub_tag_0+"]");
  if ( tracing > 0 ) DebugN("Nbr of Structured(multiple) Pubs [O/W=T]  are '"+pub_str_1+"' [tags="+pub_tag_1+"]");
  
  for ( idx = 1 ; idx <= dynlen(pub_str_all) ; ++idx )
  {
    // DebugN ( "Publication Structure" , pub_str_all[idx][1] , pub_str_all[idx][4] , pub_str_all[idx][5] ,
    //                                    pub_str_all[idx][2] , configName , strException , pub_str_all[idx][3] );
           
    fwDIP_publishStructure ( pub_str_all[idx][1] , pub_str_all[idx][4] , pub_str_all[idx][5] ,
                             pub_str_all[idx][2] , configName , strException , pub_str_all[idx][3] );
  }
}
...
shape tree = getShape ( treename );
  ...
  // tree.setColumnWidthMode();
  tree.setResizeEnabled(true);
