...
//   				if(strlen(referenceParts[2])>0)//Subscription name defined
//   				{
...
//   				{
//   					config[fwPeriphAddress_OPCUA_SUBSCRIPTION] = referenceParts[2];
//   					config[fwPeriphAddress_OPCUA_KIND] = "";
//   					config[fwPeriphAddress_OPCUA_VARIANT] = referenceParts[3];
//    				config[fwPeriphAddress_ROOT_NAME] = referenceParts[4];
//   				}          
...
// DebugN("configTypeData[1]:",configTypeData[1]);
    		switch(configTypeData[1])
    		{
		        case fwPeriphAddress_TYPE_IEC:
    				...
    				config[i][fwPeriphAddress_IEC_SUBINDEX] = dsAddressVal[o]; o++; //_address.._subindex
       ...
       /*OPC UA1
        ...
        Server$$Variant$Item */
    				...
    				config[i][fwPeriphAddress_DATATYPE] = dsAddressVal[o]; o++; //_address.._datatype
    				config[i][fwPeriphAddress_DIRECTION] = dsAddressVal[o]; o++; //_address.._direction
    				config[i][fwPeriphAddress_OPCUA_LOWLEVEL] = dsAddressVal[o]; o++; //_address.._lowlevel
    				config[i][fwPeriphAddress_ACTIVE] = dsAddressVal[o]; o++; //_address.._active
    				config[i][fwPeriphAddress_OPCUA_POLL_GROUP] = dsAddressVal[o]; o++; //_address.._poll_group
        referenceParts = strsplit(reference, "$");
	...
	if (dpExists(dpe))
		...
		if (dynlen(configParameters) >= 1)
			...
			switch (configParameters[fwPeriphAddress_TYPE])
				{
				case fwPeriphAddress_TYPE_IEC:
				             fwPeriphAddress_setIEC(dpe, configParameters, exceptionInfo);         
					break;
				case fwPeriphAddress_TYPE_MODBUS:
...
//  DebugN("lowLevel: "+lowLevel, "direction: "+configParameters[fwPeriphAddress_DIRECTION]);
...
// DebugN("fwPeriphAddress_setOPCUA",
//                            "\ndpe:",dpe, 
//                            "\nopcServerName,  ",opcServerName, 
//                            "\ndriverNum,  ",driverNum, 
//                            "\nopcItemName,  ",opcItemName, 
//                            "\nopcSubscription, ",opcSubscription,
//                            "\nopcKind, ",opcKind,
//                            "\nopcVariant, ",opcVariant,
//                            "\ndatatype,  ",datatype, 
//                            "\nmode,  ",mode, 
//                            "\npollGroup, ",pollGroup);   
...
fwPeriphAddress_setIEC(string dpe, dyn_string dsParameters, dyn_string& exceptionInfo)
{
  int driverNum, addressSubindex, dataType;
	...
	addressReference = dsParameters[FW_PARAMETER_FIELD_ADDRESS];
	dir = dsParameters[FW_PARAMETER_FIELD_MODE];
