...
global string _fwConfigurationDB_fileVersion_fwConfigurationDB_Hierarchies_ctl="5.0.2";
...
DebugTN("fwNode_createLogical",name,parent);
...
DebugTN("CheckSaveDpTypes");
	...
	for (int j=1;j<=dynlen(elementNames);j++) {
	    ...
	    // Fix FWCDB-1068: Exclude non-supported dpetypes {DPEL_DYN_BLOB, DPEL_DYN_BLOB_STRUCT} 
	    if((elementTypes[j] == DPEL_DYN_BLOB) || (elementTypes[j] == DPEL_DYN_BLOB_STRUCT)) { 
		fwConfigurationDB_genericNotify("Excluding DYN_BLOB-typed DPType-Element " + dpTypes[i]+elementNames[j], 
						"","", 2001, PRIO_WARNING);
		break; 
	    }
