...
global string _fwConfigurationDB_fileVersion_fwConfigurationDB_Utils_ctl="5.0.2";

// for progress dialog...
global bool       g_fwConfigurationDB_closeProgressDialog=FALSE;
global bool       g_fwConfigurationDB_progressDialogClosed=FALSE;
                                ...
                                bool dbRollbackOnError=true)
{
    ...
    if (fwConfigurationDB_pauseOperation) {
        DebugTN("fwConfigurationDB: Operation paused");
        while (fwConfigurationDB_pauseOperation) {
            delay(0,500);
        }
        DebugTN("fwConfigurationDB: Operation resumed");
    }
...
void _fwConfigurationDB_progressDialogThread()
{
    dyn_float df;
    dyn_string ds;
    g_fwConfigurationDB_closeProgressDialog=FALSE;
    g_fwConfigurationDB_progressDialogClosed=FALSE;
    fwConfigurationDB_pauseOperation=FALSE;
    fwConfigurationDB_abortOperation=FALSE;
...
//    ChildPanelOnCentralModalReturn("fwConfigurationDB/fwConfigurationDB_ProgressDialog.pnl","CDB Progress",makeDynString(),df,ds);
    ...
    g_fwConfigurationDB_closeProgressDialog=FALSE;
    g_fwConfigurationDB_progressDialogClosed=TRUE;
    fwConfigurationDB_pauseOperation=FALSE;
    fwConfigurationDB_abortOperation=FALSE;
}
...
{
    g_fwConfigurationDB_closeProgressDialog=TRUE;
  // fix FWCORE-3122 FWCDB-1074
    for (int i=1;i<=10;i++) {
      if (g_fwConfigurationDB_progressDialogClosed) break; // already closed...
      //DebugTN("fwConfigurationDB_closeProgressDialog()","Waiting for progress dialog to close...",i);
      delay(0,100);
    }   
    delay(0,100);// for security
}
...
{
    errClass notification;
    notification=makeError("fwConfigurationDB", prio, ERR_CONTROL, errcatEntry, s1, s2, s3);
