...
*/
global string _fwConfigurationDB_fileVersion_fwConfigurationDB_DeviceConfiguration_ctl="5.0.2";
...
DebugTN("Got values from PVSS",dynlen(values));    
    ...
    if (dynlen(idsSmall)>0) {
	sql="INSERT INTO CONFIG_VALUES(IPROP_ID,PVALUE) VALUES(:IPROP_ID,:PVALUE)";
	data[1]=idsSmall;
	data[2]=valSmall;
	DebugTN("Executing",sql, "with "+dynlen(valSmall)+" values");
	_fwConfigurationDB_executeDBBulkCmd(sql,g_fwConfigurationDB_DBConnection,data, exceptionInfo,TRUE, makeDynInt(0,STRING_VAR));
...
//	DebugTN(configurationData[1],configurationData[2],configurationData[8]);
	if (dynlen(configurationData) && dynlen(configurationData[1])) {
		...
		for (int j=1;j<=totalRanges;j++) {
			...
			alarmLimits[j][fwAlertConfig_ALERT_LIMIT_CLASS] = _fwConfigurationDB_NodeNameWithoutSystem(alarmClasses[j]);
		    } else {
...
*/
    ...
    bool finished=false;
    int dpesProcessed=0;
    //int dpesInLoop=100000;
    //int dpesInLoop=20000;
    int dpesInLoop=5000;
    int nLoops=0;
