	...
	string part = "")
{
	...
	// ATLAS: force update FSM UI cache for object
	if (isATLAS()) 
    fwUi_getDisplayInfo(makeDynString(domain, obj));
...
int _fwFsmUi_getLockLayer(bit32 statusBits, string state)
{
  int locknum;
...
//DebugN("ShowIt lock", myDomain, myObj, state, statusBits);
	...
	locknum = _fwFsmUi_getLockLayer(statusBits, state);
...
_fwFsmUi_changeLayer(int num, bit32 statusBits, string state)
{
  ...
  string bgColor;
    ...
    bgColor = _fwFsmUi_getLockBgColor(num, statusBits, state);
		setValue("lock"+num,"backCol",bgColor);
	}
}

int _fwFsmUi_getDevLockLayer(bit32 statusBits)
{
  int num = 0;
...
//DebugTN("fwUi_show_mode Lock Returning!",myDomain, myObj);
	...
	statusBits = fwFsmUi_getOwnModeBits();
  num = _fwFsmUi_getDevLockLayer(statusBits);
	_fwFsmUi_changeDeviceLayer(num, statusBits);
}

string _fwFsmUi_getDevLockBgColor(int num, bit32 statusBits, string widget)
{
  string bgColor = "_3DFace";
...
string bgColor;
  ...
  bgColor = _fwFsmUi_getDevLockBgColor(num, statusBits, widget);
	setValue(widget,"backCol",bgColor);
}
...
//	DebugN("changeMode of "+obj);
	...
	// update AC privileges for current user and domain
	if (isATLAS()) fwUi_getAccess(domain);
  
  panelName = fwFsmUi_getModesPanel(isCU);
...
*/
  ...
  if (isATLAS()) 
  {
    // show access rights needed
    //
    string accessRights;
    dyn_string items = makeDynString("","");
    fwFsmTree_getNodeAccessControl($domain, accessRights);
    if (accessRights!="")
    {
      items = strsplit(accessRights,"|");
      if (items[1][0]=="!") items[1] = substr(items[1], 1);
    }
    setValue("priv","text", items[1]);
    if (dynlen(items)==2) setValue("expert_priv", "text", items[2]);
    else setValue("expert_priv", "text", items[1]);
  }
}
...
//        setValue("","enabled",0);
  ...
  if (isATLAS()) {
    ...
    information("FSM UI: Sending "+cmd+" to object "+$domain+"::"+$obj+". User:"+userName);
    // end ATLAS
  }
...
string fwFsmUi_getModesPanel(int cu)
{
  string name, panel;
  string path;
  ...
  if(cu)
    ...
    name = "fwFsmDevModes.pnl";
  panel = "/fwFSMuser/"+name;
