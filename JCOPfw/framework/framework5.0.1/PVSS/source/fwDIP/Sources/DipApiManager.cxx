...
class DPCreateListener : public HotLinkWaitForAnswer
{
    private:
	  bool m_ready;
	public:
	...
	protected:
		void hotLinkCallBack(DpHLGroup &){
			m_ready = true;
		}
};
...
void DipApiManager::run()
	...
	if (dipFactory == NULL){
		...
		////////////////
        // FWDIP-58 Create the DIP configuration datapoint if needed.
		//        This call has no effect if the datapoint exists already.
		{
			DPCreateListener*  dpcl = new DPCreateListener();
			DpTypeId dpTypeId;
			if (!Manager::getTypeId("_FwDipConfig", dpTypeId)){
				PVSSERROR("Could not obtain DIP config DP Type _FwDipConfig, please reinstall the FwDIP component.");
			}else{
			  if(! Manager::dpCreate(DipApiResources::getConfigRootName(), dpTypeId, dpcl))
			  {
				  PVSSERROR("Could not create DIP Client Config datapoint '"<< DipApiResources::getClientSourceName()<<"'");
			  }
			}

			// We now wait for the callback, confirming the DIP Client config has been created
			for(int i = 0 ; i< 50 ; i++){
				Manager::dispatch();
			  if(dpcl->isReady()){
				  break;
			  ...
			  if(i > 49){
				  PVSSERROR("Could not create DIP Client config datapoint after 50 attempts - API manager will stop - please create the DP manually");
			  }
			}
		}
		// End of FWDIP-58 
	...
	if ((DipApiResources::getBrowseRefreshName().len()			 == 0) ||
		...
		if ((Manager::getId(DipApiResources::getBrowseRefreshName(),			browseRefreshDpID) == PVSS_FALSE) ||
			...
			(Manager::getId(DipApiResources::getRefreshActionStatusResponseName(),refreshActionStatusID) == PVSS_FALSE)){
				PVSSERROR("DIP API manager could not get DPE ids needed to configure DIP browser");
			} else {
