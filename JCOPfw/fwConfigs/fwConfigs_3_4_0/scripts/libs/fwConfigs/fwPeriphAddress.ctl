			...
			switch (configParameters[fwPeriphAddress_TYPE])
				...
				case fwPeriphAddress_TYPE_MODBUS:
					if (dynlen(configParameters) == FW_PARAMETER_FIELD_NUMBER)
						...
						fwException_raise(exceptionInfo, "ERROR", "The address configuration data is not valid: incorrect number of parameters.", "");
					break;
				...
				case fwPeriphAddress_TYPE_OPCCLIENT:
					...
					{
						throwError(err);
						fwException_raise(exceptionInfo, "ERROR","fwPeriphAddress_set(): Could not set active attribute", "");
					}
					break;
				...
				case fwPeriphAddress_TYPE_DIMCLIENT:
					...
					if(dpExists(dimConfigDp))
					{
						...
						{
							fwException_raise(exceptionInfo, "ERROR","fwPeriphAddress_set(): The specified DP is not a DIM config DP", "");
							return;
						}
					}
					else
					{
						fwException_raise(exceptionInfo, "ERROR","fwPeriphAddress_set(): The specified DIM config DP does not exist", "");
						return;
					}
				...
				case fwPeriphAddress_TYPE_DIP:
					...
					if(result > 0)
					{
						...
						if(dpExists(dipConfigDp))
						{
							...
							{
								fwException_raise(exceptionInfo, "ERROR","fwPeriphAddress_set(): The specified DP is not a DIP config DP", "");
								return;
							}
						}
						else
						{
							fwException_raise(exceptionInfo, "ERROR","fwPeriphAddress_set(): The specified DIP config DP does not exist", "");
							return;
						}
