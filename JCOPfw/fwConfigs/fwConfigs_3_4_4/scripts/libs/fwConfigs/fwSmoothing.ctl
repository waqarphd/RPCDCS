...
{
	string configString;
  dyn_string dsDpAttr, dsAttrVal, dsTypesAttr, dsTypesVal, dsTempVal;
...
//   DebugN("attrib len: "+dynlen(dsAttrVal)+" Expected: ",3*length);
  //get the parameters variables
  if(dynlen(dsAttrVal)>0)
  {  
	  k=1;
	  for(i=1 ; i<=length ; i++)
	  {
//       if(i>1150)DebugN(dpes[i]);
      iSmoothType = dsAttrVal[k];
			switch(iSmoothType)
			{
				case DPATTR_VALUE_SMOOTH:
				case DPATTR_VALUE_REL_SMOOTH:
				...
				default:
  				...
  				timeInterval[i] = 0; k++;      
  				fwException_raise(exceptionInfo, "ERROR",
  									"_fwSmoothing_getParameters(): Smoothing procedure " + dsAttrVal[j] + " on dpe '"+dpes[i]+"' not suppported", "");
				break;      
			}
...
*/
/*
_fwSmoothing_getParametersOld(string dpe, bool isArchiveConfig, int &smoothProcedure, float &deadband, float &timeInterval, dyn_string &exceptionInfo)
{
	string configString;
