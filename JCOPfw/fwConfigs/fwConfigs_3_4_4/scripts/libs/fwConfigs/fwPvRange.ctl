...
@param exceptionInfo	details of any errors are returned here
*/
fwPvRange_getMany(	dyn_string &pes, dyn_bool &configExists, dyn_float &minValue,
			...
			dyn_bool &ignoreOutside, dyn_bool &inclusiveMin, dyn_bool &inclusiveMax, dyn_string &exceptionInfo)
{
  ...
  if(dynlen(exceptionInfo))
  {
	 			fwException_raise(localExceptionInfo, "ERROR", "fwPvRange_getMany(): Could not get some of the attributes. Function aborted.","");      
      return;
  }
	length = dynlen(dpes);
	for(i=1; i<=length; i++)
